...
public class CreateReservationFormBackStepTest {
    ...
    @Test
    public void testVolverDeStep2MantieneDatos() {
        // Ir a crear reserva
        ...
        // Volver solo una vez (de step2 a step1)
        onView(withId(R.id.toolbar_navigation)).perform(click());
        onView(isRoot()).perform(EspressoTestUtils.waitForView(withId(R.id.btnNextStep), 5000));

        // Comprobar que los campos siguen igual
        onView(withId(R.id.datePickerButton)).check(matches(withText(org.hamcrest.Matchers.not("Seleccionar fecha"))));
        onView(withId(R.id.startTimeButton)).check(matches(withText(org.hamcrest.Matchers.not("Hora inicio"))));
        onView(withId(R.id.endTimeButton)).check(matches(withText(org.hamcrest.Matchers.not("Hora fin"))));
        // Puedes añadir más checks según los valores que hayas puesto
    }
